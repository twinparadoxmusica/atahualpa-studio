...
const TOPIC_COPY = {
  fr: { label:'Objet de votre demande', placeholder:'Choisissez un sujet', music:'Cours de musique', recording:'Enregistrement & streaming', acoustics:'Acoustique & insonorisation', other:'Autre' },
  en: { label:'Subject of your request', placeholder:'Choose a subject', music:'Music lessons', recording:'Recording & streaming', acoustics:'Acoustics & soundproofing', other:'Other' },
  es: { label:'Motivo de tu consulta', placeholder:'Elegí un tema', music:'Clases de música', recording:'Grabación & streaming', acoustics:'Acústica & insonorización', other:'Otro' },
  it: { label:'Oggetto della richiesta', placeholder:'Scegli un argomento', music:'Lezioni di musica', recording:'Registrazione & streaming', acoustics:'Acustica & insonorizzazione', other:'Altro' },
};

const ContactForm = () => {
  const { t, locale } = useLanguage();
  const topicCopy = TOPIC_COPY[locale] || TOPIC_COPY.fr;
  ...
  return <>
    <section className="contact-hero" aria-labelledby="contact-hero-title"><Container variant="default" className="contact-hero__inner">
      <span className="contact-hero__eyebrow">{t('contact.hero.eyebrow')}</span><h1 id="contact-hero-title" className="contact-hero__title">{t('contact.hero.title')}</h1><p className="contact-hero__lede">{t('contact.hero.lede')}</p>
    ...
    <section className="contact-form-section"><Container variant="narrow"><SectionHeader eyebrow={t('contact.form.eyebrow')} title={t('contact.form.title')} lede={t('contact.form.lede')} /><div className="contact-form-card">
      {submitted ? <p className="contact-form__success">{t('contact.form.success')}</p> : <form onSubmit={handleSubmit} className="contact-form" noValidate>
        ...
        <label className="contact-form__field"><span>{topicCopy.label}</span><select name="topic" defaultValue="" required><option value="" disabled>{topicCopy.placeholder}</option><option value="music">{topicCopy.music}</option><option value="recording">{topicCopy.recording}</option><option value="acoustics">{topicCopy.acoustics}</option><option value="other">{topicCopy.other}</option></select></label>
